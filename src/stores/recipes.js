...
export const useRecipeStore = defineStore('recipes', {
  ...
  actions: {
    ...
    async fetchRecipes(params) {
      ...
      const data = await fetchWithAuth(`/recipes?${urlParams.toString()}`);
    ...
    async fetchCookLogs(id) {
      const data = await fetchWithAuth(`/recipes/${id}/logs`);
      this.cookLogs = data || [];
      return this.cookLogs;
    },
    async fetchComments(id) {
      const data = await fetchWithAuth(`/recipes/${id}/comments`);
      this.comments = data || [];
